...
if (isSetupEnabled()) {
  ...
  app.get("/setup", function(req, res) {
    res.send(
      "<html><body><H1>InitializeMerchant Token</H1><p>This token requires this server to be running so it can " +
  ...
  app.get("/toPayPal/:env", allErrorsAreBelongToUs, function(req, res) {
    res.redirect(
      paypal.redirect(
        req.params.env,
        APP_REDIRECT_URL,
        !!req.query.returnTokenOnQueryString
      )
    );
  });
}
...
app.get("/returnFromPayPal", function(req, res) {
  console.log("in /returnFromPayPal");
  console.log("req.query", req.query);
  paypal.completeAuthentication(req.query, APP_SECURE_IDENTIFIER, function(
    error,
    destinationUrl
  ) {
    console.log("callback of completeAuthentication");
    if (error) {
      console.error(
        util.format(
          "Failed to handle returnFromPayPal (%s): %s\n%s",
          error.env || "unknown environment",
          error.message,
          error.stack
        )
      );
...
var server = app.listen(process.env.PORT || 3000, function() {
  ...
  console.log(
    "PayPal Retail SDK Service listening at http://%s:%s",
    host,
    port
  );
});
...
function configurePayPal(paypalAccountRegion) {
  ...
  if (PAYPAL_CUSTOM_ENVIRONMENT) {
    try {
      var info = JSON.parse(
        new Buffer(PAYPAL_CUSTOM_ENVIRONMENT, "base64").toString("utf8")
      );
      for (var envI = 0; envI < info.length; envI++) {
        console.log("Configuring", info[envI].name, "environment");
        info[envI].returnUrl =
          info[envI].returnUrl || combineUrl(ROOT_URL, "returnFromPayPal");
        info[envI].refreshUrl =
          info[envI].refreshUrl || combineUrl(ROOT_URL, "refresh");
...
function validateEnvironment() {
  ...
  if (!APP_SECURE_IDENTIFIER) {
    error(
      "The APP_SECURE_IDENTIFIER value is missing from the environment. It should be set to a reasonably long set of random characters (e.g. 32)"
    );
  }
  if (!APP_REDIRECT_URL && !process.env.SETUP_ENABLED) {
    error(
      "Either APP_REDIRECT_URL (for third party merchant login) or SETUP_ENABLED (for first party token generation) must be set in the environment."
    );
  }
  if (!PAYPAL_LIVE_CLIENTID && !PAYPAL_SANDBOX_CLIENTID) {
    error(
      "The server must be configured for sandbox, live, or both. Neither PAYPAL_LIVE_CLIENTID or PAYPAL_SANDBOX_CLIENTID is set in the environment."
    );
  } else {
    if (!PAYPAL_LIVE_CLIENTID) {
      ...
      if (!PAYPAL_LIVE_SECRET) {
        error(
          "PAYPAL_LIVE_CLIENTID is set, but PAYPAL_LIVE_SECRET is not. The app needs the client id and secret to function."
        );
      } else {
        hasLive = true;
      }
    ...
    if (!PAYPAL_SANDBOX_CLIENTID) {
      ...
      if (!PAYPAL_SANDBOX_SECRET) {
        error(
          "PAYPAL_SANDBOX_CLIENTID is set, but PAYPAL_SANDBOX_SECRET is not. The app needs the client id and secret to function."
        );
      } else {
        hasSandbox = true;
      }
  ...
  if (!APP_REDIRECT_URL) {
    warn(
      "The APP_REDIRECT_URL value is missing from the environment. You will only be able to use this service to authenticate via /setup."
    );
  }
  if (!ROOT_URL) {
    warn(
      "The environment variable ROOT_URL should be set to the root URL of this server, such as http://mypaypalsdkserver.herokuapp.com"
    );
  }
}
...
function allErrorsAreBelongToUs(req, res, next) {
  if (errors && errors.length) {
    res.send(
      "<html><body><h1>Configuration Errors</h1><ul><li>" +
        errors.join("</li><li>") +
        "</li></pre></body>"
    );
  } else {
...
function showStartupMessage() {
  ...
  if (!ROOT_URL) {
    ...
    if (isSetupEnabled()) {
      console.log(
        " * To generate a token for your account, open the following URL in a browser:\n *"
      );
      if (hasLive) {
        console.log(
          " *     LIVE:    " + combineUrl(ROOT_URL || "/", "setup/live")
        );
      }
      if (hasSandbox) {
        console.log(
          " *     SANDBOX: " + combineUrl(ROOT_URL || "/", "setup/sandbox")
        );
      }
    }
    if (APP_REDIRECT_URL) {
      ...
      if (PAYPAL_LIVE_CLIENTID) {
        console.log(
          " *     LIVE:    " + combineUrl(ROOT_URL || "/", "toPayPal/live")
        );
      }
      if (PAYPAL_SANDBOX_CLIENTID || true) {
        console.log(
          " *     SANDBOX: " + combineUrl(ROOT_URL || "/", "toPayPal/sandbox")
        );
      }
      console.log(
      ...
      console.log(
        " *     " +
          APP_REDIRECT_URL +
          (APP_REDIRECT_URL.indexOf("?") >= 0 ? "&" : "?") +
          "sdk_token=[what you give to InitializeMerchant]"
      );
      console.log(
        " *\n * Your return url on developer.paypal.com must be set to:\n *"
      );
      console.log(
        " *     " + combineUrl(ROOT_URL, "returnFromPayPal") + "\n *"
      );
    }
  }
  console.log(
