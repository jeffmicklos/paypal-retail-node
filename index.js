...
module.exports = {
  ...
  configure: function configure(environment, options) {
    if (!options.clientId) {
      throw new Error(
        'Missing clientId for PayPal environment "' + environment + '"'
      );
    }
    if (!options.secret) {
      throw new Error(
        'Missing secret for PayPal environment "' + environment + '"'
      );
    }
    if (!options.returnUrl) {
      throw new Error(
        'Missing returnUrl for PayPal environment "' + environment + '"'
      );
    }
    if (!options.refreshUrl) {
      throw new Error(
        'Missing refreshUrl for PayPal environment "' + environment + '"'
      );
    }
    if (!options.scopes) {
      options.scopes =
        "openid email phone profile address https://uri.paypal.com/services/paypalhere https://api.paypal.com/v1/payments/.* https://uri.paypal.com/services/paypalattributes/business";
    }
  ...
  queryAvailableScopes: function(env, callback) {
    ...
    wreck.post(
      oauthUrl(env),
      {
        ...
        headers: {
          Authorization:
            "Basic " + new Buffer(cfg.clientId + ":").toString("base64"),
      ...
      function(err, rz, payload) {
        if (!err && payload && payload.client_metadata) {
          ...
          if (!payload) {
            err = new Error(
              "PayPal services did not return scopes for your application."
            );
          }
          var requiredScopes = cfg.scopes.split(" "),
            missing = [];
          ...
          if (missing.length) {
            err = new Error(
              "Your application is missing the following required scopes: " +
                missing.join(" ")
            );
            err.missing = missing;
  ...
  redirect: function(env, finalUrl, returnTokenOnQueryString) {
    ...
    if (env == module.exports.SANDBOX) {
      return util.format(
        "https://www.sandbox.paypal.com/webapps/auth/protocol/openidconnect/v1/authorize?response_type=code&client_id=%s&scope=%s&redirect_uri=%s&state=%s",
        encodeURIComponent(cfg.clientId),
        encodeURIComponent(cfg.scopes),
        encodeURIComponent(cfg.returnUrl),
        encodeURIComponent(
          JSON.stringify([env, finalUrl, !!returnTokenOnQueryString])
        )
      );
    } else if (env.indexOf("stage2") === 0) {
      return util.format(
        "https://www.%s.stage.paypal.com/webapps/auth/protocol/openidconnect/v1/authorize?response_type=code&client_id=%s&scope=%s&redirect_uri=%s&state=%s",
        env,
        encodeURIComponent(cfg.clientId),
        encodeURIComponent(cfg.scopes),
        encodeURIComponent(cfg.returnUrl),
      ...
      return util.format(
        "https://www.paypal.com/webapps/auth/protocol/openidconnect/v1/authorize?response_type=code&client_id=%s&scope=%s&redirect_uri=%s&state=%s",
        encodeURIComponent(cfg.clientId),
        encodeURIComponent(cfg.scopes),
        encodeURIComponent(cfg.returnUrl),
        encodeURIComponent(
  ...
  refresh: function(query, app_secure_identifier, callback) {
    ...
    decrypt(query.token, app_secure_identifier, function(e, plain) {
      ...
      try {
        ...
        if (!cfg) {
          return callback(
            new Error("Invalid environment " + encodeURIComponent(info[0]))
          );
        }
        wreck.post(
          url,
          {
            ...
            headers: {
              Authorization:
                "Basic " +
                new Buffer(cfg.clientId + ":" + cfg.secret).toString("base64"),
              "Content-Type": "application/x-www-form-urlencoded"
            }
          },
          function(err, rz, payload) {
  ...
  completeAuthentication: function(query, app_secure_identifier, callback) {
    ...
    if (!app_secure_identifier) {
      throw new Error(
        "app_secure_identifier parameter is required to complete authentication."
      );
    }
    if (!callback || typeof callback !== "function") {
      throw new Error(
        "completeAuthentication requires a callback parameter that is a function."
      );
    }
    if (query.error) {
      console.log("completeAuthentication query.error", query.error);
      return callback(
        new Error(
          util.format(
            "Login with PayPal Error! %s: %s",
            query.error,
    ...
    console.log("no error in completeAuthentication", query);

    var state = JSON.parse(query.state);
    if (!state || state.length < 2) {
      throw new Error(
        'The "state" parameter is invalid when trying to complete PayPal authentication.'
      );
    }
    var env = state[0];
    ...
    var url = tsUrl(env);
    var cfg = configs[env];
    console.log("completeAuthentication about to wreck POST");
    wreck.post(
      url,
      {
        payload: util.format(
          "grant_type=authorization_code&code=%s&redirect_uri=%s",
          encodeURIComponent(query.code),
          encodeURIComponent(cfg.returnUrl)
        ),
        json: "force",
        headers: {
      ...
      function(err, rz, payload) {
        console.log("completeAuthentication wreck POST callback", payload);
        if (err) {
          err.env = env;
          return callback(err);
        }
        ...
        var refreshUrl =
          cfg.refreshUrl + (cfg.refreshUrl.indexOf("?") >= 0 ? "&" : "?");
        encrypt(
          JSON.stringify([env, payload.refresh_token]),
          app_secure_identifier,
          function encryptionDone(e, v) {
            if (e) {
              e.env = env;
              return callback(e);
            }
            ...
            if (returnTokenOnQueryString) {
              returnUrl +=
                "access_token=" +
                encodeURIComponent(tokenInformation[0]) +
                "&expires_in=" +
                encodeURIComponent(tokenInformation[1]) +
                "&refresh_url=" +
                encodeURIComponent(tokenInformation[2]) +
                "&env=" +
              ...
              returnUrl +=
                ...
                ":" +
                encodeURIComponent(
                  new Buffer(JSON.stringify(tokenInformation)).toString(
                    "base64"
                  )
                );
            }
...
function tsUrl(env) {
  ...
  if (env == module.exports.SANDBOX) {
    ...
    url = util.format(
      "https://www.%s.stage.paypal.com:12714/v1/identity/openidconnect/tokenservice",
      env
    );
  }
  return url;
...
function oauthUrl(env) {
  ...
  if (env == module.exports.SANDBOX) {
    ...
    url = util.format(
      "https://www.%s.stage.paypal.com:11888/v1/oauth2/token",
      env
    );
  }
  return url;
